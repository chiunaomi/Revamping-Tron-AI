...
from BasicBotMovement import BasicBot
...
class TronModel(object):
    ...
    def __init__(self,cell_length,width,height):
        ...
        self.bot = BasicBot(self.screen,10,(self.width/2),(self.height/2),"u",(255,140,0))
    ...
    def in_cell(self):
        ...
        for cell in self.cell_lst:
            if self.player2.x in cell.xrange and self.player2.y in cell.yrange:
                ...
                break
        for cell in self.cell_lst:
            if self.bot.x in cell.xrange and self.bot.y in cell.yrange:
                self.bot.current_cell = cell
                break
    ...
    def update(self):
        ...
        last_seen_bot = self.bot.current_cell
        ...
        if self.bot.current_cell != last_seen_bot:
            self.player_paths.append(last_seen_bot)
        ...
        if self.player2.current_cell in self.player_paths:
            self.end_game("ORANGE ",(255,140,0))
        if self.bot.current_cell in self.player_paths:
